...
document.addEventListener('DOMContentLoaded', function () {
    ...
    searchInput.addEventListener('keyup', function () {
        ...
        // Make AJAX request if query is not empty
        if (query.length >= 2) {
            fetch(`/search/?q=${query}`, {
                ...
                .then(data => {
                    ...
                    if (data.courses.length || data.centers.length || data.categories.length) {
                        let resultsHtml = '';

                        // Display recommended centers
                        if (data.centers.length > 0) {
                            resultsHtml += `<p class="searching-heading">Recommended by EduCompass</p>`;
                            data.centers.forEach(center => {
                                resultsHtml += `
                                <a href="#">
                                  <div class="searching-rec-wrapper">
                                    <img src="${center.logo}" alt="" />
                                    <div class="searching-rec-text-wrapper">
                                      <p class="searching-company-name">
                                        ${center.name}
                                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                          <!-- SVG Content -->
                                        </svg>
                                      </p>
                                      <span>${center.course_count}ta kurs • ${center.location}</span>
                                    </div>
                                  </div>
                                  <div class="searching-rec-rating">
                                    <svg width="20" height="20">
                                      <!-- SVG Content for rating -->
                                    </svg>${center.rating}
                                  </div>
                                </a>`;
                            });
                        ...
                        if (data.courses.length > 0) {
                            resultsHtml += `<p class="searching-heading">Results for "${query}"</p>`;
                            data.courses.forEach(course => {
                                resultsHtml += `<a href="#"><div>${course.name} - Level: ${course.level}</div></a>`;
                            });
                        }
